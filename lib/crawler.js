...
class Crawler {
  ...
  async crawl() {
    ...
    await this._waitFor();
    const result = this._scrape();
    const [screenshot, cookies, links] = await Promise.all([
      this._screenshot(),
      this._getCookies(),
  ...
  async _scrape() {
    ...
    await this._addScriptTag();

    return this._page.evaluate(
      this._options.evaluatePage,
      this._options.evaluatePageArgs
    );
  }
   ...
   */
  async _addScriptTag() {
    if (!this._options.scriptTag) return;

    //use for to handle async in loop
    for (const script of this._options.scriptTag) {
      await this._page.addScriptTag(script);
    }
  }
