...
class HCCrawler extends EventEmitter {
  ...
  constructor(browser, options) {
    ...
    this._browser.on(
      "disconnected",
      () => void this.emit(HCCrawler.Events.Disconnected)
    );
  }
    ...
    await Promise.all(
      map(isArray(options) ? options : [options], async _options => {
        ...
        if (mergedOptions.evaluatePage)
          mergedOptions.evaluatePage = `(${mergedOptions.evaluatePage})(args)`;
  ...
  _checkAllowedDomains(options) {
    ...
    if (
      ...
      checkDomainMatch(options.deniedDomains, hostname)
    )
      return false;
    if (
      options.allowedDomains &&
      !checkDomainMatch(options.allowedDomains, hostname)
    )
      return false;
    return true;
  }
